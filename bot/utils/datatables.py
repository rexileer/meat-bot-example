...
def generate_minced_meat_batch_mix_groups(start=None, end=None) -> dict:
    """Группировка выхода продукции по рецептам за день.

    Учитываем только замесы, которые попали на склад ГП в интервале [start, end]
    (статус "storage"). Массу берем из статуса "palletizing_end" (weight_raw).
    """
    groups = {}
    ...
    # Берем замесы, завершенные в интервале (work_is_finished)
    finished_qs = MincedMeatBatchStatus.objects.filter(
        status__codename="work_is_finished", created_at__gte=start, created_at__lte=end
    )
    storage_mix_ids = list(
        finished_qs.values_list("minced_meat_batch_mix_id", flat=True)
    )
    if not storage_mix_ids:
        return groups

    mixes = MincedMeatBatchMix.objects.filter(id__in=storage_mix_ids)
    ...
    for mix in released_mixes:
        ...
        recipe_name = re.sub("ВторФарш ", "", recipe_name)
        palletizing = mix.statuses.filter(status__codename="palletizing_end").first()
        if not palletizing or not palletizing.additional_data:
            continue
        weight_raw = palletizing.additional_data.get("weight_raw", 0)
...
def second_minced_additional_data(mix: SecondMincedMeat):
    ...
    try:
        return int(palletizing.additional_data["weight_raw"])
    except Exception:
        return 0


def generate_meat_batch_datatable(date) -> BytesIO:
    date_format = "%d.%m.%Y"
    start = date.replace(hour=7, minute=0, second=0, microsecond=0)
    end = date + timedelta(days=1)

    raw_material_list, data = RawMeatBatch.used_and_filtred_raw_meats_today(start, end)
    minced_meat_batch_groups = generate_minced_meat_batch_mix_groups(start, end)
    ...
    weight_all_second_minced_meat = sum(
        [
            second_minced_additional_data(i)
            for i in second_minced_meats
    ...
    for i, raw_meat_batch in enumerate(raw_meat_batches, start=1):
        ...
        i_tb = i + 2
        ws[f"A{i_tb}"].value = i
        ws[f"D{i_tb}"].value = raw_meat_batch.organization
        ws[f"E{i_tb}"].value = raw_meat_batch.raw_material.name
        ws[f"F{i_tb}"].value = raw_meat_batch.buh_accounting
        ...
        ws[f"M{i_tb}"].value = (
            ""
            if raw_meat_batch.condition != "frozen"
            else meterial_params.defrost
            if meterial_params
            else ""
        )
        ...
        ws[f"H{i_tb}"].value = (
            ...
            if meterial_params
            and meterial_params.defrost
            and raw_meat_batch.condition == "frozen"
            else old
        )
        ws[f"C{i_tb}"].value = (
            meterial_params.second_minced_meat_exit if meterial_params else 0
        )
        ws[f"G{i_tb}"].value = old
        ...
        if separation_status:
            ...
            ws[f"K{i_tb}"].value = separator_name_mapping.get(
                separation_status.additional_data["separator_name"], 1
            )
    ...
    ws[f"D{minced_last_row + 1}"].value = (
        f"{(minced_weight_sum / weight_sum) * 100:.1f}" if weight_sum else ""
    )
    ws[f"E{minced_last_row + 1}"].value = (
        f"{(weight_all_second_minced_meat / weight_sum) * 100:.1f}"
        if weight_sum
        else ""
    )
    ws[f"D{minced_last_row}"].number_format = numbers.FORMAT_NUMBER_00
    ws.merge_cells(f"D{minced_last_row + 2}:E{minced_last_row + 2}")
    ws[f"D{minced_last_row + 2}"].value = (
        minced_weight_sum + weight_all_second_minced_meat
    )
