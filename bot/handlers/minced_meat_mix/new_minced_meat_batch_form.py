...
async def set_raw_materials_handler(message: Message, state: FSMContext):
    ...
    if raw_material:
        ...
        raw_meat_batches = RawMeatBatch.objects.filter(
            weight__gt=0, raw_material_id=raw_material.raw_material.pk, condition="chilled", is_future_batch=False
        ).all()
        raw_meat_batches = [
            meat_batch
        ...
        await message.answer("Выберите партию для фарша", reply_markup=set_raw_meat_batch_keyboard(raw_meat_batches))
        await NewMincedMeatBatchForm.set_raw_meat_batch.set()
    else:
...
async def set_weight_handler(message: Message, state: FSMContext):
    state_data = await state.get_data()
    raw_meat_batch = RawMeatBatch.objects.filter(pk=state_data["current_raw_meat_batch_id"]).first()
    ...
    if weight <= raw_meat_batch.weight:
        ...
        await message.answer(f'"Такой массы нет на складе. Максимальная масса: {raw_meat_batch.weight} кг"')
...
async def minced_meat_batch_line(call: types.CallbackQuery, state: FSMContext):
    ...
    work_line = int(call.data.split("-")[1])
    is_shocker = True if work_line == 2 else False
    await state.update_data(work_line=work_line)
    await state.update_data(is_shocker=is_shocker)
...
async def set_arrival_date_handler(message: Message, state: FSMContext, roles: RolesModel):
    ...
    minced_meat_batch = await MincedMeatBatch.objects.acreate(
        ...
        is_shocker=state_data["is_shocker"],
